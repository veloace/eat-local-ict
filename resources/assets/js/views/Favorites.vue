        ...
        <div class="container has-text-white has-background-translucent" style="margin-top: 25px;min-height: 100vh">
            ...
            <div class="tile is-ancestor" v-if="!loading && items.length>0">
                <div class="tile is-parent is-4" v-for="item in items" :key="item.id">
                    <article class="tile is-child box is-primary has-background-black-bis">
                        ...
                        <div v-if="item.user_comment">
                            <p  class="heading has-text-warning">You said..
                            </p>
                            <blockquote class="has-text-warning is-italic has-text-centered">"{{item.user_comment}}"</blockquote>
                            <hr>
                        </div>
                        ...
                        <button class="button is-danger is-outlined block is-fullwidth" @click="removeFromSaved(item)">Remove From Favorites</button>
                    </article>
                </div>
            </div>
            <div v-else-if="!loading && items.length<1" class="hero is-fullheight">
                <header>
                    ...
                    <p class="has-text-centered has-text-warning">It doesn't look like you have favorited any local places yet..</p>
    ...
    export default {
        ...
        methods: {
            ...
            removeFromSaved(place) {
                if(confirm("Remove "+place.name+" from your favorites?")) {


                    this.loading = true;
                    axios.post('/api/places/lists/favorites/delete', {id: place.id})
                        .then((response) => {
                            this.loading = false;
                            this.$root.showNotification('Removed '+place.name+' from your favorites.', 'success');
                        ...
                        .catch((error) => {
                            this.scrollToTop();
                            this.loading = false;
                            this.$root.showNotification('We encountered an error and were unable to delete this item from your favorites. If this problem persists, try refreshing the page.', 'danger');
                        });
                }
            },
            scrollToTop() {
